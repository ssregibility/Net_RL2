...
Notes
- ResNet*XX*-S*s*U*u* denotes ResNet*XX* model adapted with our method having *s* rank of shared filter basis and *u* rank of unique filter basis. Use ```--shared_rank=s``` and ```--unique_rank=u``` for evaluating pretrained models.
