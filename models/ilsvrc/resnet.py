...
# original Bottleneck block with shared parameters
class BottleneckBlock_SharedSingle(nn.Module):
    expansion = 4

    def __init__(self, in_planes, planes, shared_basis, stride=1):
        super(BottleneckBlock_SharedSingle, self).__init__()
        self.conv1 = nn.Conv2d(in_planes, planes, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.shared_basis = shared_basis
        # self.conv2 = nn.Conv2d(planes, planes, kernel_size=3,
    ...
    def forward(self, x):
        out = F.relu(self.bn1(self.conv1(x)))
        # out = F.relu(self.bn2(self.conv2(out)))
        out = F.relu(self.bn2(self.shared_basis(out)))
        out = self.bn3(self.conv3(out))
        out += self.shortcut(x)
        out = F.relu(out)
        return out
...
# Proposed ResNet sharing bottleneck as a basis for each residual block group
class ResNet_BottleneckSharedSingle(nn.Module):
    def __init__(self, block_basis, block_original, num_blocks, num_classes=1000):
        super(ResNet_BottleneckSharedSingle, self).__init__()
        self.in_planes = 64
        ...
        self.shared_basis_1 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=False)
        self.layer1 = self._make_layer(block_basis, block_original, 64, num_blocks[0], self.shared_basis_1, stride=1)
        
        self.shared_basis_2 = nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1, bias=False)
        self.layer2 = self._make_layer(block_basis, block_original, 128, num_blocks[1], self.shared_basis_2, stride=2)
        
        self.shared_basis_3 = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=False)
        self.layer3 = self._make_layer(block_basis, block_original, 256, num_blocks[2], self.shared_basis_3, stride=2)
        
        self.shared_basis_4 = nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def _make_layer(self, block_basis, block_original, planes, blocks, shared_basis_1, stride=1):
        layers = []
        ...
        self.in_planes = planes * block_original.expansion
        for _ in range(1, blocks):
            layers.append(block_basis(self.in_planes, planes, shared_basis_1))
...
# A model with a shared basis in each residual block group.
def ResNet50_SharedSingle():
    return ResNet_BottleneckSharedSingle(BottleneckBlock_SharedSingle, BottleneckBlock, [3, 4, 6, 3])
